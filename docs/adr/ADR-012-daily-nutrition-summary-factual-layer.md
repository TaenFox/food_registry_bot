...
День определяется по локальной дате пользователя, но с пользовательским смещением начала nutrition-day.

Для расчёта:

- берётся `user.timezone` из текущей модели пользователя;
- берётся `nutrition_day_start_hour` из пользовательских summary settings;
- определяется локальный интервал `[HH:00, HH:00 следующего дня)` для нужной даты;
...
- записи с локальным временем от `00:00` до `HH:59:59` предыдущего порога относятся к предыдущему nutrition-day;
- записи с локальным временем от `HH:00` и позже относятся к текущему nutrition-day.
